...
// run alevin-fry for quantification with splici index
 process alevin_fry_MR3 {
    ...
    if [ "${params.protocol}" = "10XV2_avoid" ]
    then
    ...
    elif [ "${params.protocol}" = "10XV3_avoid" ]
    then
        alevin-fry generate-permit-list --input ${runId}_ALEVIN_fry_map -d fw --unfiltered-pl /nfs/production/irene/ma/users/nnolte/whitelist/3M-february-2018.txt  --output-dir ${runId}_ALEVIN_fry_quant  --min-reads 0
    else
