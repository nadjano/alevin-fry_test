...
process droplet_qc_plot{
    ...
    input:
        set val("type"), file("test_counts_mtx") Channel.from(ALEVIN_MTX_FOR_QC, ALEVIN_FRY_MTX_FOR_QC)
...
process remove_empty_drops {
    ...
    input:
        set val(type), file(countsMtx) from [ALEVIN_MTX_FOR_EMPTYDROPS, ALEVIN_FRY_MTX_FOR_EMPTYDROPS]
