...
process get_STAR_mapping {
    ...
    """
    MR_${runId}_${mode}=\$(grep "Reads Mapped to Gene: Unique ${mode}" "${runId}_STAR_tmpSolo.out/${mode}/Summary.csv" | awk '{split(\$0, array, ","); print array[2]}' | cut -c 1-4  > ${mode}_MR_${runId}.txt)
